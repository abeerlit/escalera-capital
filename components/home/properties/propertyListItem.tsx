...
export const PropertyListItem: React.FC<PropertyListItemProps> = ({
    ...
    return (
        <div className='flex bg-white mt-2 relative shadow-[0_0_5px_rgba(0,0,0,0.3)] h-40 overflow-hidden'>
            <div className='relative h-full w-fit min-w-[160px] max-w-[160px]'>
                <img src={image} alt={title} className='w-full h-full object-cover' />
                <button className='absolute top-2 right-2 cursor-pointer' onClick={() => setIsFavorite(!isFavorite)}>
            ...
            <div className='px-4 py-2 flex flex-col  flex-1 min-w-0'>
                <div className='font-bold text-lg truncate' title={title}>{title}</div>
